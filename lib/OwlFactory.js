...
class OwlFactory {
   ...
   */
  constructor(routes, port, env, { mongodbConfig, otherDbConfig }) {
    this.app = express();
    ...
    this.initDatabase(mongodbConfig || otherDbConfig);
  }
  ...
  initDatabase(config) {
    if (config?.url !== null) {
      ...
      if (ustring[0] !== 'mongodb') {
        throw new Error("The constructor's first optional param supports mongodb database only`");
      }
